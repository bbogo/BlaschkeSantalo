function str = APWLloydGeogram(samples,bbox,lb,ub,niter,pl)
...
for i=1:niter
   ...
   % end
    ...
    if nargin>=6
		[Areas, Centroids, Moments] = VoronoiGeogram(coords,bbox,pl);
		drawnow
		%pause
	else
	...
	for j=1:(frac*size(Centroids,1))
		%if norm(coords(j,:)-Centroids(j,:))>tol
		    ncomp = ncomp+1;
		    ap = APWfunc(samples(:,ind(j)));
			
			
			[newsam(:,ind(j)),out] = MoveCenter4(samples(:,ind(j)),Centroids(ind(j),:),@(x)APWfunc(x),@(x) APWjac(x),lb,ub);
	...
	%		newsam(:,j) = samples(:,j);
	%	end
		errs(j) = norm(fun(newsam(:,ind(j)))-ap);
	end
	dif = max(errs);
